...
public class TestThreadPoolManager implements BeanFactoryAware {
    ...
    //线程池维护线程所允许的空闲时间
    private final static int MKEEP_ALIVE_TIME=0;
    ...
    /**
     * 订单的缓存队列,当线程池满了,则将订单存入到此缓冲队列
     */
    Queue<Object> msgQueue=new LinkedBlockingDeque<>();

    /**
     * 当线程池的容量满了,执行下面代码,将订单加入到缓存对列
     */
   final RejectedExecutionHandler handler=new RejectedExecutionHandler() {
    ...
    /**创建线程池*/
    final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(CORE_POOL_SIZE, MAX_POOL_SIZE, MKEEP_ALIVE_TIME, TimeUnit.SECONDS, new ArrayBlockingQueue(WORK_QUEUE_SIZE), this.handler);
